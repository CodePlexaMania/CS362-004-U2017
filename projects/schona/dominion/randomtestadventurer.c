...
void AdvEffect(int player, struct gameState *post)
{
	printf("Iteration #%d:\n", iteration++);
	...
	int effect = AdventurerEffect(player, post);

	//Test 1: Check AdventurerEffect returns 0
	AssertTrue(effect == 0, "AdventurerEffect(): return 0");
	
	//Test 2: Check that player has gained 2 cards
	AssertTrue(post->handCount[player] == pre.handCount[player] + 2, "Player gained 2 cards");

	//Test 3: Check the players deckCount decreased at minimum by 2 
	AssertTrue(post->deckCount[player] <= pre.deckCount[player] + 2, "Gained cards came from player's own pile");

	//Test 4: Check the drawn cards are both treasure cards
	...
	//Test 5: Check to make sure that discard count is greater than or equal 0
	AssertTrue(post->discardCount[player] >= pre.discardCount[player], "Discard count >= 0");
			 			 
	//Test 6: check numActions is same as we are only calling Adventurer Effect which does not discard
	AssertTrue(post->playedCardCount == pre.playedCardCount, "Player's numActions has not changed");
	
	//Test 7: check played card count is same as we are only calling Adventurer Effect which does not discard
	AssertTrue(post->numActions == pre.numActions, "Player's playedCardCount has not changed");
	
	//Test 8: check coins is same as we are only calling Adventurer Effect which does not discard
	AssertTrue(post->coins == pre.coins, "Player's coins has not changed");
	
	//Test 9: Check that other than the above tests, nothing else changed
